...
def load_data(file_path):
    ...
    if len(data.columns) < 2:
        raise InvalidDataError("Error: The dataset must contain at least one attribute and one decision column.")

    if len(numeric_cols) != len(data.columns) - 1:
        raise InvalidDataError("Error: Invalid dataset. Expected n-1 numerical columns and a decision one (nth).")
...
def discretize_data(data, use_secondary_criterion=False, verbose=True):
    ...
    print(f"Generated {len(object_pairs)} object pairs with different decisions.") if verbose else None
    ...
    while True:
        best = {'gain': 0, 'attr': None, 'cut': None, 'new_separations': set(), 'cuts_count': float('inf')}
            ...
            for cut in possible_cuts:
                ...
                if use_secondary_criterion:
                    if gain > 0 and (cuts_count < best['cuts_count'] or (cuts_count == best['cuts_count'] and gain > best['gain'])):
                        best.update({'gain': gain, 'attr': attr, 'cut': cut, 'new_separations': new_separations, 'cuts_count': cuts_count})
                else:
                    if gain > best['gain']:
                        best.update({'gain': gain, 'attr': attr, 'cut': cut, 'new_separations': new_separations, 'cuts_count': cuts_count})
        ...
        if verbose:
            print(f"Added cut {best['cut']} on attribute '{best['attr']}', separated {best['gain']} new pairs.")
    ...
    stats['coverage'] = stats['separated_pairs'] / stats['total_pairs'] if stats['total_pairs'] else 0
    stats['average_cuts_per_attribute'] = stats['cuts_added'] / len(attributes) if attributes else 0
